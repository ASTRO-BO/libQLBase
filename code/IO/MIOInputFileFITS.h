...
class MIOInputFileFITS : public MIOInputFile {
	...
	public:
		...
		virtual Long_t GetNextRowPeriod(UInt_t timeColumnNumber, Long_t pos_first, DOUBLE_T end_time);

		virtual DOUBLE_T* GetTime(UInt_t timeColumnNumber, ULong_t start, ULong_t dim);

		virtual DOUBLE_T GetTime(UInt_t timeColumnNumber, ULong_t pos);

		virtual UInt_t GetIndexFirstColumn() { return 1;};

		virtual UInt_t GetIndexFirstRow() { return 1;};
	...
	public:
		...
		//##	UChar_t* gridlcdi = inputFile->Read_TBYTE(GRIDLCDI, first_pos_new, last_pos_new);<br>
		//## 	if(gridlcdi == kPointerNull) return inputFile->GetStatus();<br>
		//## Esempio di lettura di una colonna in formato variabile: <br>
		//## 	UChar_t* aclatcon = tempFile.Read_TBYTE(ACLATCON, j+1, j+1, 12);<br>
		...
		virtual UChar_t* Read_TBYTE(int ncol, long frow, long lrow, Long_t nelements = 0);
		...
		virtual UShort_t* Read_TUSHORT(int ncol, long frow, long lrow, Long_t nelements = 0);

		virtual UInt_t* Read_TUINT(int ncol, long frow, long lrow, Long_t nelements = 0);

		virtual ULong_t* Read_TULONG(int ncol, long frow, long lrow, Long_t nelements = 0);
